...
func MongoUserCreate(ctx *core.Context) (string, error) {
	data := mdb.User{
		ID:        1,
		Username:  "laya",
		Nickname:  "layasugar",
...
func MongoUserUpdate(ctx *core.Context, mid string) error {
	var data mdb.User
...
func MongoUserSelect(ctx *core.Context, mid string) (*mdb.User, error) {
	var data mdb.User
	id, _ := primitive.ObjectIDFromHex(mid)
	filter := bson.E{Key: "_id", Value: id}
...
func MongoUserDel(ctx *core.Context, mid string) error {
	var data mdb.User
	id, _ := primitive.ObjectIDFromHex(mid)
	filter := bson.E{Key: "_id", Value: id}
