...
func MysqlUserCreate(ctx *laya.WebContext, params User) error {
	return dao.Orm(ctx).Model(&User{}).Create(&params).Error
}

func MysqlUserUpdate(ctx *laya.WebContext, phone string, nickName string) error {
	return dao.Orm(ctx).Model(&User{}).Where("mobile = ?", phone).Updates(map[string]interface{}{
		"nickname": nickName,
	}).Error
}

func MysqlUserSelect(ctx *laya.WebContext, phone string) (*User, error) {
	var u = &User{}
	err := dao.Orm(ctx).Where("mobile = ?", phone).First(u).Error
	return u, err
}

func MysqlUserDel(ctx *laya.WebContext, phone string) error {
